...
TEXTS = {
    "en": {
        ...
        "set_default_description": "Set which saved profile is used when --profile is omitted and the fixed environment credential set is absent (WEEX_API_KEY, WEEX_API_SECRET, and WEEX_API_PASSPHRASE).",
        "set_default_profile_help": "Profile name or id to use as the default profile",
        "clear_default_help": "Clear the default profile",
        "clear_default_description": "Remove the current default saved profile. Saved-profile commands must then specify --profile; direct contract/spot commands may still use WEEX_API_KEY, WEEX_API_SECRET, and WEEX_API_PASSPHRASE.",
    ...
    "zh": {
        ...
        "set_default_description": "设置在省略 --profile 且未配置固定三项环境变量 WEEX_API_KEY、WEEX_API_SECRET、WEEX_API_PASSPHRASE 时自动使用的账号。",
        "set_default_profile_help": "要设为默认账号的账号名或 id",
        "clear_default_help": "清除默认账号",
        "clear_default_description": "移除当前默认账号；saved-profile 命令之后必须显式指定 --profile，但合约/现货直连接口仍可使用 WEEX_API_KEY、WEEX_API_SECRET、WEEX_API_PASSPHRASE。",
