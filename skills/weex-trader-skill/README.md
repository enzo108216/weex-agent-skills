...
- in natural-language flows, ask the user to reply with exactly `user_confirmation.reply_text`; this value is standardized across UI languages, even when the explanatory prompt is English. Do not ask them to copy `intent_id`, `risk_signature`, or longer phrases such as "confirm order"
...
- `preview-order` returns `user_confirmation.reply_text` for the human reply; the required reply text is standardized across UI languages, while `reply_instruction` may be localized. The response still returns `intent_id` plus `risk_signature` for internal execution binding.
