...
class VaultManagerAppTests(unittest.TestCase):
    ...
    def test_vault_layout_routes_inline_help_through_shared_helpers(self) -> None:
        source = inspect.getsource(app.VaultManagerApp._build_layout)

        self.assertIn("_pack_title_with_help(", source)
        self.assertGreaterEqual(source.count("_grid_label_with_help("), 4)

    def test_vault_manager_defines_help_icon_and_bubble_helpers(self) -> None:
